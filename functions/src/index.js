...
async function getAccessToken() {
    const auth = Buffer.from(`${SPOTIFY_CLIENT_ID.value()}:${SPOTIFY_CLIENT_SECRET.value()}`).toString("base64");
 ...
 */
export const searchSpotify = onCall({ secrets: [SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, SPOTIFY_REFRESH_TOKEN] }, async (request) => {
    const query = request.data.query;
    if (!query)
        return [];
    const token = await getAccessToken();
    const res = await fetch("https://api.spotify.com/v1/search?" +
        new URLSearchParams({ q: query, type: "track", limit: "5" }), { headers: { "Authorization": `Bearer ${token}` } });
    const data = await res.json();
...
export const addSongToSpotify = onDocumentCreated({
    ...
    try {
        ...
        if (!trackUri && data.title) {
            ...
            const searchRes = await fetch("https://api.spotify.com/v1/search?" +
                new URLSearchParams({ q: query, type: "track", limit: "1" }), { headers: { "Authorization": `Bearer ${token}` } });
